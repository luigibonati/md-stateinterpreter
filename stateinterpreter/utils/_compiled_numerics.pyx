...
@cython.boundscheck(False)
@cython.wraparound(False)
cpdef _evaluate_kde_args(real[:,:] points, real[:,:] dataset, real bwidth_norm, real[:] logweights, real _logweights_norm, dtype):
    ...
    for i in range(x_dim):
        ...
        for j in prange(data_dim, nogil=True):
            res += math.exp(_get_arg(i, j, points, dataset, bwidth_norm, norm, logweights[j], d))
        result[i] = res
    return np.asarray(result)

@cython.boundscheck(False)
@cython.wraparound(False)
cdef real _get_arg(Py_ssize_t i, Py_ssize_t j, real[:,:] points, real[:,:] dataset, real bwidth_norm, real norm, real logweight, int d) nogil:
    cdef Py_ssize_t k  
    cdef real arg = 0
    for k in range(d):
...
@cython.boundscheck(False)
@cython.wraparound(False)
def _evaluate_logkde_args(real[:,:] points, real[:,:] dataset, real bwidth_norm, real[:] logweights, real _logweights_norm, dtype):
    cdef int data_dim = dataset.shape[0]
    cdef int x_dim = points.shape[0]
    cdef int d = points.shape[1]
    cdef Py_ssize_t i, j
    ...
    for i in prange(x_dim, nogil=True):
        result[i] = _get_log_arg(i, points, dataset, bwidth_norm, norm, logweights, d, data_dim)
    return np.asarray(result)  

@cython.boundscheck(False)
@cython.wraparound(False)
cdef real _get_log_arg(Py_ssize_t i, real[:,:] points, real[:,:] dataset , real bwidth_norm, real norm, real[:] logweights, int d, int data_dim) nogil:
    cdef real alpha = -INFINITY
    cdef real r = 0
    cdef real arg
    cdef Py_ssize_t j
    for j in range(data_dim):
        arg = _get_arg(i, j, points, dataset, bwidth_norm, norm, logweights[j], d)
        if arg != - INFINITY:
            if arg <= alpha:
